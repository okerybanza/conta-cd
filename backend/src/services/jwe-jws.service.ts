...
import { EncryptJWT, SignJWT, jwtDecrypt, jwtVerify, importJWK, importPKCS8, importSPKI } from 'jose';
import { createPrivateKey, createPublicKey, KeyObject } from 'crypto';
import logger from '../utils/logger';

export interface JWEHeader {
  alg: string; // Algorithm for key encryption
  enc: string; // Algorithm for content encryption
  kid?: string; // Key ID
...
export interface JWSHeader {
  alg: string; // Algorithm for signature
  kid?: string; // Key ID
  typ?: string; // Type (usually "JWT")
  cty?: string; // Content Type
}
...
export interface VisaEncryptionConfig {
  // Symmetric (API Key + Shared Secret)
  apiKey?: string;
  sharedSecret?: string;
  encryptionKeyId?: string; // KID for encryption
  
  // Asymmetric (RSA PKI)
  visaEncryptionPublicKey?: string; // Visa's public key (PEM format)
  visaEncryptionKeyId?: string; // Visa's encryption certificate ID
  clientEncryptionPrivateKey?: string; // Client's private key (PEM format)
  clientEncryptionKeyId?: string; // Client's encryption certificate ID
}
...
export interface VisaSigningConfig {
  ...
  // Asymmetric (RSA PKI)
  clientSigningPrivateKey?: string; // Client's private key for signing (PEM format)
  clientSigningKeyId?: string; // Client's signing certificate ID
  visaSigningPublicKey?: string; // Visa's public key for verification (PEM format)
  visaSigningKeyId?: string; // Visa's signing certificate ID (usually "715EA257")
}

export class JWEJWSService {
  /**
   ...
   */
  async createJWESymmetric(
    payload: any,
    config: VisaEncryptionConfig
  ): Promise<string> {
    ...
    try {
      ...
      logger.error('Error creating JWE (symmetric)', {
        error: error.message,
        stack: error.stack,
      });
   ...
   */
  async createJWEAsymmetric(
    payload: any,
    config: VisaEncryptionConfig,
    encryptionMethod: 'A128GCM' | 'A256GCM' = 'A128GCM'
  ): Promise<string> {
    ...
    try {
      ...
      const visaPublicKey = await importSPKI(
        config.visaEncryptionPublicKey,
        'RSA-OAEP-256'
      );
      ...
      const jwe = await new EncryptJWT(payloadObject)
        .setProtectedHeader({
      ...
      logger.error('Error creating JWE (asymmetric)', {
        error: error.message,
        stack: error.stack,
      });
      throw new Error(`JWE encryption failed: ${error.message}`);
   ...
   */
  async createVisaDirectMLEPayload(
    payload: any,
    config: VisaEncryptionConfig,
    encryptionMethod: 'A128GCM' | 'A256GCM' = 'A128GCM'
  ): Promise<{ encData: string }> {
  ...
  async decryptJWESymmetric(
    ...
    try {
      ...
      logger.error('Error decrypting JWE (symmetric)', {
        error: error.message,
        stack: error.stack,
      });
   ...
   */
  async decryptJWEAsymmetric(
    jweOrResponse: string | { encData: string },
    config: VisaEncryptionConfig
  ): Promise<any> {
    ...
    try {
      ...
      const clientPrivateKey = await importPKCS8(
        config.clientEncryptionPrivateKey,
        'RSA-OAEP-256'
      );

      const { payload } = await jwtDecrypt(jweString, clientPrivateKey);
      ...
      logger.error('Error decrypting JWE (asymmetric)', {
        error: error.message,
        stack: error.stack,
      });
      throw new Error(`JWE decryption failed: ${error.message}`);
   ...
   */
  async createJWSSymmetric(
    payload: any,
    config: VisaSigningConfig
  ): Promise<string> {
    ...
    try {
      ...
      logger.error('Error creating JWS (symmetric)', {
        error: error.message,
        stack: error.stack,
      });
  ...
  async createJWSAsymmetric(
    ...
    try {
      // Importer la clé privée client (PEM format)
      const clientPrivateKey = await importPKCS8(
        config.clientSigningPrivateKey,
        'PS256'
      );
      ...
      logger.error('Error creating JWS (asymmetric)', {
        error: error.message,
        stack: error.stack,
      });
      throw new Error(`JWS signing failed: ${error.message}`);
  ...
  async verifyJWSSymmetric(
    ...
    try {
      ...
      const { payload } = await jwtVerify(jws, secretKey);
      return payload;
    } catch (error: any) {
      logger.error('Error verifying JWS (symmetric)', {
        error: error.message,
        stack: error.stack,
      });
  ...
  async verifyJWSAsymmetric(
    ...
    try {
      // Importer la clé publique Visa (PEM format)
      const visaPublicKey = await importSPKI(
        config.visaSigningPublicKey,
        'PS256'
      );

      const { payload } = await jwtVerify(jws, visaPublicKey);
      return payload;
    } catch (error: any) {
      logger.error('Error verifying JWS (asymmetric)', {
        error: error.message,
        stack: error.stack,
      });
      throw new Error(`JWS verification failed: ${error.message}`);
   ...
   */
  async createSignedJWE(
    payload: any,
    encryptionConfig: VisaEncryptionConfig,
    signingConfig: VisaSigningConfig,
    useAsymmetric: boolean = true
  ): Promise<string> {
    try {
      // 1. Créer le JWE
      let jwe: string;
      ...
      logger.error('Error creating signed JWE', {
        error: error.message,
        stack: error.stack,
      });
      throw error;
   ...
   */
  async verifyAndDecryptSignedJWE(
    signedJWE: string,
    encryptionConfig: VisaEncryptionConfig,
    signingConfig: VisaSigningConfig,
    useAsymmetric: boolean = true
    ...
    try {
      ...
      logger.error('Error verifying and decrypting signed JWE', {
        error: error.message,
        stack: error.stack,
      });
      throw error;
...
export default new JWEJWSService();
