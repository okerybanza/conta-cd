...
class DepreciationService {
    ...
    async create(companyId, data, userId) {
        ...
        const depreciation = await database_1.default.depreciations.create({
            data: {
                company_id: companyId,
                ...
                purchaseDate,
                purchase_amount: new library_1.Decimal(data.acquisitionCost),
            ...
            include: {
                accounts_depreciations_asset_account_idToaccounts: true,
                accounts_depreciations_depreciation_account_idToaccounts: true,
            },
        });
    ...
    async getById(companyId, depreciationId) {
        const depreciation = await database_1.default.depreciations.findFirst({
            where: {
                id: depreciationId,
                company_id: companyId,
            },
    ...
    async list(companyId, filters = {}) {
        ...
        const depreciations = await database_1.default.depreciations.findMany({
            where,
            include: {
                accounts_depreciations_asset_account_idToaccounts: true,
                accounts_depreciations_depreciation_account_idToaccounts: true,
    ...
    async update(companyId, depreciationId, data) {
        ...
        if (!depreciation.is_active && data.isActive !== true) {
        ...
        const purchaseAmount = Number(depreciation.purchase_amount);
        const depreciationRate = Number(depreciation.depreciation_rate);
        ...
        if (data.depreciationMethod || data.usefulLife || data.depreciationRate) {
            const method = data.depreciationMethod || depreciation.depreciation_method;
        ...
        const updated = await database_1.default.depreciations.update({
    ...
    async calculateMonthlyDepreciation(companyId, depreciationId) {
        ...
        if (depreciation.depreciation_method === 'linear') {
            return this.calculateLinearMonthlyDepreciation(purchaseAmount, usefulLife);
        }
        else {
            const purchaseAmount = Number(depreciation.purchase_amount);
            const rate = Number(depreciation.depreciation_rate || 0);
    ...
    userId) {
        ...
        const entry = await journalEntry_service_1.default.create(companyId, {
            ...
            lines: [
                {
                    accountId: depreciation.depreciation_account_id,
                    description: `Amortissement - ${depreciation.description}`,
                    debit: monthlyDepreciation,
                    credit: 0,
                },
                {
                    accountId: depreciation.asset_account_id,
                    description: `Amortissement - ${depreciation.description}`,
                    debit: 0,
    ...
    async generateDepreciationTable(companyId, depreciationId) {
        const depreciation = await this.getById(companyId, depreciationId);
        const purchaseDate = depreciation.purchase_date;
        const purchaseAmount = Number(depreciation.purchase_amount);
        const depreciationRate = Number(depreciation.depreciation_rate);
        // Calculer usefulLife à partir de depreciationRate (usefulLife = 100 / rate)
        const usefulLife = depreciationRate > 0 ? 100 / depreciationRate : 0;
        const method = depreciation.depreciation_method;
    ...
    async delete(companyId, depreciationId) {
        ...
        await database_1.default.depreciations.delete({
            where: { id: depreciationId },
        });
    ...
    async processMonthlyDepreciations() {
        ...
        for (const depreciation of depreciations) {
            try {
                // Vérifier si l'actif est complètement amorti
                const purchaseAmount = Number(depreciation.purchase_amount);
                const accumulated = await this.calculateAccumulatedDepreciation(depreciation.company_id, depreciation.id);
                if (accumulated >= purchaseAmount) {
                    logger_1.default.info(`Depreciation ${depreciation.id} is fully depreciated, skipping`);
                    results.push({
                        depreciationId: depreciation.id,
                        company_id: depreciation.company_id,
                        success: true,
                        skipped: true,
                ...
                const entry = await this.generateDepreciationEntry(depreciation.company_id, depreciation.id, period);
                logger_1.default.info(`Depreciation entry generated: ${entry.id}`, {
                    depreciationId: depreciation.id,
                    company_id: depreciation.company_id,
                    period,
