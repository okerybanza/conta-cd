...
class BankReconciliationService {
    ...
    async importBankStatement(companyId, accountId, statementData) {
        ...
        const existing = await database_1.default.bankStatement.findFirst({
            where: {
                companyId,
        ...
        const statement = await database_1.default.bankStatement.create({
            data: {
                companyId,
                accountId,
                statementNumber: statementData.statementNumber,
    ...
    async reconcileStatement(statementId) {
        const statement = await database_1.default.bankStatement.findUnique({
        ...
        for (const bankTx of statement.transactions) {
            ...
            if (bestMatch) {
                await database_1.default.bankTransaction.update({
                    where: { id: bankTx.id },
                    data: {
                        isReconciled: true,
        ...
        await database_1.default.bankStatement.update({
            where: { id: statementId },
            data: {
    ...
    async getBankStatement(statementId, companyId) {
        const statement = await database_1.default.bankStatement.findFirst({
            where: {
                id: statementId,
                companyId,
                deletedAt: null,
    ...
    async listBankStatements(companyId, accountId) {
        ...
        const statements = await database_1.default.bankStatement.findMany({
            where,
            include: {
                account: true,
                _count: {
