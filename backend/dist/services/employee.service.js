...
class EmployeeService {
    ...
    async create(companyId, data, userId) {
        ...
        const existing = await database_1.default.employees.findFirst({
            where: {
        ...
        if (existing) {
            throw new error_middleware_1.CustomError(`Un employé avec le numéro ${data.employeeNumber} existe déjà`, 400, 'EMPLOYEE_NUMBER_EXISTS');
        }
        const employee = await database_1.default.employees.create({
            data: {
        ...
        const event = new domain_event_1.EmployeeCreated({ userId, companyId, timestamp: new Date() }, employee.id, employee.employee_number, companyId);
        event_bus_1.eventBus.publish(event);
        logger_1.default.info(`Employee created: ${employee.id}`, { companyId, employeeId: employee.id });
        return employee;
    }
    ...
    async list(companyId, filters = {}) {
        ...
        const [employees, total] = await Promise.all([
            database_1.default.employees.findMany({
                where,
                skip,
                take: limit,
                orderBy: {
            ...
            database_1.default.employees.count({ where }),
        ]);
        return {
            data: employees,
            pagination: {
    ...
    async update(companyId, employeeId, data, userId) {
        ...
        if (data.hireDate !== undefined)
            updateData.hire_date = new Date(data.hireDate);
        if (data.terminationDate !== undefined) {
            ...
            if (data.terminationDate) {
                ...
                const terminationEvent = new domain_event_1.EmployeeTerminated({ userId, companyId, timestamp: new Date() }, employeeId, new Date(data.terminationDate), undefined);
                event_bus_1.eventBus.publish(terminationEvent);
            }
        }
        ...
        if (data.salaryFrequency !== undefined)
            updateData.salary_frequency = data.salaryFrequency;
        if (data.bankAccount !== undefined)
            updateData.bank_account = data.bankAccount;
        ...
        if (data.socialSecurityNumber !== undefined)
            updateData.social_security_number = data.socialSecurityNumber;
        ...
        const updated = await database_1.default.employees.update({
            where: { id: employeeId },
            data: updateData,
        });
        // Publier l'événement de mise à jour (DOC-04)
        if (Object.keys(changes).length > 0) {
            const event = new domain_event_1.EmployeeUpdated({ userId, companyId, timestamp: new Date() }, employeeId, changes);
            event_bus_1.eventBus.publish(event);
        }
        logger_1.default.info(`Employee updated: ${employeeId}`, { companyId, employeeId });
        return updated;
    }
    // Supprimer un employé (soft delete) - DOC-04 : aucune suppression physique
    async delete(companyId, employeeId, userId) {
        const employee = await this.getById(companyId, employeeId);
        // Publier événement de sortie avant suppression logique
        const terminationEvent = new domain_event_1.EmployeeTerminated({ userId, companyId, timestamp: new Date() }, employeeId, new Date(), 'Suppression demandée');
        event_bus_1.eventBus.publish(terminationEvent);
        await database_1.default.employees.update({
            where: { id: employeeId },
            data: {
                deleted_at: new Date(),
                status: 'terminated',
        ...
        logger_1.default.info(`Employee deleted: ${employeeId}`, { companyId, employeeId });
        return { success: true };
    }
}
