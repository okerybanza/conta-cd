...
exports.handlePayrollValidated = handlePayrollValidated;
exports.handleEmployeeContractCreated = handleEmployeeContractCreated;
exports.handleEmployeeContractTerminated = handleEmployeeContractTerminated;
...
async function handlePayrollValidated(event) {
    ...
    if (existingEntry) {
        ...
        logger_1.default.debug(`Accounting entry already exists for payroll (created atomically): ${event.payrollId}`, {
    ...
    logger_1.default.warn(`Accounting entry missing for payroll (should have been created atomically): ${event.payrollId}`, {
        payrollId: event.payrollId,
        companyId,
    });
 ...
 */
async function handleEmployeeContractCreated(event) {
    try {
        const { companyId } = event.metadata;
        ...
        const activeContracts = await database_1.default.employee_contracts.findMany({
            where: {
        ...
        if (activeContracts.length > 0) {
            logger_1.default.error('Multiple active contracts found for employee', {
        ...
        logger_1.default.error('Error handling EmployeeContractCreated event', {
            contractId: event.contractId,
            error: error.message,
        });
    }
 ...
 */
async function handleEmployeeContractTerminated(event) {
    try {
        const { companyId } = event.metadata;
        const activeContracts = await database_1.default.employee_contracts.findMany({
            where: {
        ...
        logger_1.default.error('Error handling EmployeeContractTerminated event', {
            contractId: event.contractId,
            error: error.message,
        });
    }
}
//# sourceMappingURL=hr.handlers.js.map